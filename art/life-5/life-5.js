...
export const handler = buildFunctionEndpoint(() => {
  ...
  while (emptyPoints.length) {
    ...
    openLines.forEach((line) => {
      ...
      const adjustedY = y + randomInt(-10, 10);

      if (adjustedY < gridHeight / 3) {
        const nextPositions = [];
      ...
      } else if (adjustedY < (gridHeight * 2) / 3) {
        const nextPositions = [
          // top left
          {
            x: x - 1,
        ...
        if (nextPositions.length === 0 || randomChance(0.25)) {
          ...
          nextPositions.forEach((position, i) => {
            if (randomChance(0.5)) {
              ...
              // if (randomChance(0.8)) {
              if (i === nextPositions.length - 1 || nextPositions.length < 2) {
